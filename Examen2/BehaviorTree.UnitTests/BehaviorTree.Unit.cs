...
namespace BehaviorTree.UnitTests
{
    ...
    public class BehaviorTree_Tests
    {
        ...
        {
            Assert.Throws<MissingMethodException>(() => Activator.CreateInstance(typeof(Composite)));
        }
        ...
        {
            Assert.Throws<MissingMethodException>(() => Activator.CreateInstance(typeof(Task)));
        }
        ...
        {
            var root = new Root(new MoveTask(() => false));
        ...
        {
            var selector = new Selector(new MoveTask(() => false), new MoveTask(() => true));
